...
export default function App() {
  ...
  const scheduleCropSave = crop => { clearTimeout(saveTimer.current); saveTimer.current = setTimeout(() => updateCrop(crop), 220); };
  ...
  const moveCanvasDrag = e => { if (!gesture.current) return; const g = gesture.current; const next = { ...g.crop, offset_x: Math.round(g.crop.offset_x + (e.clientX - g.x) * targetPixels[0] / g.width), offset_y: Math.round(g.crop.offset_y + (e.clientY - g.y) * targetPixels[1] / g.height) }; setDraft(next); };
  const endCanvasDrag = () => { if (!gesture.current) return; gesture.current = null; updateCrop(draftRef.current); };
  const wheelCanvas = e => { e.preventDefault(); const next = { ...draftRef.current, zoom: Math.max(35, Math.min(240, draftRef.current.zoom + (e.deltaY < 0 ? 5 : -5))) }; setDraft(next); scheduleCropSave(next); };
  ...
  return <div className="app">
    <header className="topbar">
      ...
      {!PUBLIC_MODE && !noAiMode && <button className="settings-button" onClick={openAiSettings} aria-label="AI 设置"><Settings size={17}/></button>}
    </header>
    ...
    <main className="workspace">
      ...
      {job && active && <>
        ...
        <section className="stage"><div className="canvas interactive" style={{aspectRatio:`${targetPixels[0]} / ${targetPixels[1]}`}} onPointerDown={startCanvasDrag} onPointerMove={moveCanvasDrag} onPointerUp={endCanvasDrag} onPointerCancel={endCanvasDrag} onWheel={wheelCanvas}><img draggable="false" style={{transform:visualTransform}} src={`${apiUrl(active.preview_url)}?barcode=${previewBarcode || '43'}&size=900&completion_mode=${completionMode}&v=${active.crop.zoom}-${active.crop.offset_x}-${active.crop.offset_y}-${target}`}/>{active.image_type !== '全身' && <div className={`safe-zone ${isSquare ? 'safe-square' : ''}`}><span>鞋子主体规范区 · {safe}</span></div>}<div className="subject-box"><span>鞋子主体</span></div>{active.keypoints?.map(point => <i key={point.name} className="keypoint" title={`${point.name} ${Math.round(point.confidence * 100)}%`} style={{left:`${point.x * 100}%`,top:`${point.y * 100}%`}}/>)}</div><div className="canvas-meta"><span>输出画布 {target}</span><span>{active.image_type === '腿模' ? '腿模靠顶 · 鞋子完整' : active.image_type === '全身' ? '人物躯体完整自适应' : `规范区 ${safe}`}</span><span>{detailLike ? '完整画布：产品细节铺满' : noAiMode ? '完整画布：不足区域留白' : '完整画布：不足时 AI 整图扩展'}</span></div></section>
    ...
    {!PUBLIC_MODE && !noAiMode && showSettings && <div className="modal-backdrop" onMouseDown={() => setShowSettings(false)}><section className="settings-modal" onMouseDown={e => e.stopPropagation()}><header><div><b>AI 识别与扩图配置</b><small>选择可直接使用的图片编辑服务</small></div><button onClick={() => setShowSettings(false)}><X size={18}/></button></header><label className="toggle"><input type="checkbox" checked={aiForm.enabled} onChange={e => setAiForm({...aiForm,enabled:e.target.checked})}/>启用 AI 完整扩图</label><label>服务类型<select value={AI_PRESETS.find(p => p.provider === aiForm.provider && p.base_url === aiForm.base_url && p.model === aiForm.model)?.id || 'compatible'} onChange={e => { const preset = AI_PRESETS.find(p => p.id === e.target.value); setAiForm({...aiForm,provider:preset.provider,base_url:preset.base_url,model:preset.model}); }}>{AI_PRESETS.map(p => <option key={p.id} value={p.id}>{p.name}｜{p.note}</option>)}</select></label><label>API 站点链接<input value={aiForm.base_url} onChange={e => setAiForm({...aiForm,base_url:e.target.value})} placeholder="https://服务地址/v1"/></label><label>图片编辑模型<input value={aiForm.model} onChange={e => setAiForm({...aiForm,model:e.target.value})} placeholder="必须支持图片编辑/完整扩图"/></label><label>API Key<input type="password" value={aiForm.api_key} onChange={e => setAiForm({...aiForm,api_key:e.target.value})} placeholder={health?.ai_configured ? '已安全保存；留空保持不变' : '输入所选服务的 API Key'}/></label><p>本地网关可免费运行，但需要自行安装图片编辑模型；Gemini 图片 API 当前需开通计费，适合低成本试用。Key 仅保存在当前 Mac 系统钥匙串。切换供应商时请填写对应的新 Key。</p><button className="save-settings" onClick={saveAiSettings}>{busy || '保存配置'}</button></section></div>}
  </div>;
}
